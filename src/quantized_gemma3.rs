...
/// Causal (optionally sliding-window) mask. Layers share a mask whenever their
/// window configuration matches, so this is built twice per forward pass rather
/// than once per layer.
fn build_mask(
    sliding_window_size: Option<usize>,
    b_sz: usize,
...
impl ModelWeights {
    ...
    pub fn forward(&mut self, x: &Tensor, index_pos: usize) -> Result<Tensor> {
        ...
        let positions = vec![seq_len - 1; b_sz];
        self.forward_positions(x, index_pos, &positions)
    }

    /// Forward a padded batch and project the last real token from each row.
    /// Right-padding is safe because causal attention prevents an earlier
    /// selected position from seeing padding to its right.
    pub fn forward_positions(
        ...
        if positions.len() != b_sz {
            candle::bail!(
                "expected one output position per batch row, got {} for batch {b_sz}",
                positions.len()
            )
        }
        if positions.iter().any(|&position| position >= seq_len) {
            candle::bail!("output position is outside the sequence")
        }
        let _enter = self.span.enter();
        ...
        // Layers alternate between sliding and full attention, so build one
        // mask per distinct window instead of one per layer.
        let mut masks: Vec<(Option<usize>, Tensor)> = Vec::new();
        if seq_len > 1 {
            for window in self.layers.iter().map(|layer| layer.sliding_window_size) {
                if !masks.iter().any(|(known, _)| *known == window) {
                    let mask = build_mask(window, b_sz, seq_len, index_pos, x.dtype(), x.device())?;
                    masks.push((window, mask));
                }
            }
        }

        for layer in self.layers.iter_mut() {
            let attention_mask = masks
                .iter()
                .find(|(window, _)| *window == layer.sliding_window_size)
        ...
        let rows = positions
            .iter()
            .enumerate()
            .map(|(row, &position)| layer_in.i((row, position, ..))?.unsqueeze(0))
            .collect::<Result<Vec<_>>>()?;
        let rows = rows.iter().collect::<Vec<_>>();
        let x = Tensor::cat(&rows, 0)?;
    ...
    pub fn snapshot_cache(&self) -> Vec<Option<(Tensor, Tensor)>> {
        self.layers
            .iter()
            .map(|layer| layer.kv_cache.clone())
            .collect()
    }

    pub fn restore_cache(&mut self, snapshot: &[Option<(Tensor, Tensor)>]) {
        for (layer, kv) in self.layers.iter_mut().zip(snapshot.iter()) {
            layer.kv_cache = kv.clone();
        }
    }

    /// Expand a batch-1 prefix cache for independent batched suffixes.
    pub fn repeat_cache(&mut self, batch: usize) -> Result<()> {
        for layer in self.layers.iter_mut() {
            if let Some((k, v)) = layer.kv_cache.take() {
                layer.kv_cache = Some((k.repeat((batch, 1, 1, 1))?, v.repeat((batch, 1, 1, 1))?));
            }
        }
        Ok(())
    }
